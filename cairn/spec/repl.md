...
Each stdin line SHALL be parsed with the same `token` and `auth` command grammar as the one-shot CLI (the account-less configuration wizard has no REPL form; bare `ortie` runs it). A parse error or a command error SHALL be reported and the loop SHALL continue, never terminating the process.
